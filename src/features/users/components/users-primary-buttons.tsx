...
export function UsersPrimaryButtons() {
  ...
  const handleExportCsv = () => {
    try {
      ...
      const headers = ['ID', 'Name', 'Email', 'Phone', 'Tenant', 'Role', 'Status', 'Auth Provider', 'Joined Date', 'Last Login']
      const csvRows = rows.map((row) => {
        ...
        const membership = u.tenantMemberships?.find(
          (m: any) => m.tenant.id === selectedTenantId
        ) || u.tenantMemberships?.find(
          (m: any) => m.tenant.id === u.primaryTenant?.id
        ) || (u.tenantMemberships?.length ? u.tenantMemberships[0] : null)

        const tenantName = membership?.tenant?.name ?? u.primaryTenant?.name ?? ''
