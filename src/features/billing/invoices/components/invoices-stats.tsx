...
export function InvoicesStats() {
  ...
  if (summary.totalPaidLastMonth > 0) {
    const percent = Math.abs((diff / summary.totalPaidLastMonth) * 100).toFixed(
      1
    )
    if (diff > 0) {
      growthComponent = (
        <span className='flex items-center text-xs text-green-500'>
          <ArrowUpRight className='mr-1 h-3 w-3' />+{percent}% from last month
        </span>
      )
    } else if (diff < 0) {
      growthComponent = (
        <span className='flex items-center text-xs text-red-500'>
          <ArrowDownRight className='mr-1 h-3 w-3' />-{percent}% from last month
        </span>
      )
    } else {
      growthComponent = (
  ...
  return (
    <div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-3'>
      ...
      <Card className='gap-2 py-4'>
        ...
        <CardContent className='px-6 pt-1 pb-4 text-left'>
